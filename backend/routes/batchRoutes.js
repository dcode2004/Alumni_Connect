    ...
    if (isExist) {
      ...
      if (isExist.isSpecialUser === "admin") {
        ...
        if (isBatchExist || isBatchStartingSameYear || isBatchEndingSameYear) {
          ...
          const isBranchExists = await Branch.findOne({ branch: "CSE" });
          if (isBranchExists) {
            ...
            let branch = "CSE"; // new branch name cse :- It will be created only once
...
router.get("/fetchBatchLists", async (req, res) => {
  try {
    const findAllBatchLists = await Batch.find({ branch: "CSE" }).select({
      studentLists: 0,
      // strength: 0,
      branch: 0,
    });
